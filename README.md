...
This model performs slightly worse than our initial model (accuracy of 72.5%), however, there is little difference in performance of the two models. We find that reducing number of neurons or epochs has little impact on performance. Therefore, it may be reasonable to assume that we should try to use a different activation functions or add hidden layers to see if this makes a difference. 
